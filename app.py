...
def simulate_generator(symbol, horizon_key, sims):
    ...
    sum_en = info.get('longBusinessSummary','無可用公司簡介')
    tr = GoogleTranslator(source='auto',target='zh-TW')
    industry = tr.translate(ind_en) if ind_en!='N/A' else 'N/A'
    summary = tr.translate(sum_en[:4000]) if sum_en else '無可用公司簡介'
    summary = summary.replace('惠丘市','新竹市')  # 微調
    ...
    finals = all_paths[:,-1]
    avg_p = finals.mean(); min_p = finals.min(); max_p = finals.max()
