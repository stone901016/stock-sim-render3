...
def simulate_generator(symbol, horizon_key, sims):
    ...
    # 只繪 2000 條路徑，其餘全用來計算統計
    draw_limit = min(sims, 2000)
    draw_chunk = max(1, draw_limit // 2000)

    for i in range(0, sims, chunk):
        cnt = min(chunk, sims - i)
        rand = np.random.normal(size=(cnt, days))
        incr = (mu - 0.5 * sigma**2) / 252 + sigma * np.sqrt(1/252) * rand
        sims_paths = current_price * np.exp(np.cumsum(incr, axis=1))
    ...
    # 繪圖：只畫前 2000 條
    sample_paths = all_paths[:draw_limit]
    x = np.arange(days + 1)
    fig, ax = plt.subplots(figsize=(14,7))
    ax.plot(x, sample_paths.T, lw=0.5, alpha=0.02, color='#007bff')
    ax.plot(x, np.mean(all_paths, axis=0), lw=3, color='#dc3545', label='平均路徑')
    ...
    if ma20>ma50 and macd_hist>0 and rsi14<70:
        advice = "趨勢偏多: 建議於股價回檔至20日均線附近分批買入，並依當前多頭動能保持持有。"
    elif ma20>ma50 and rsi14>=70:
        advice = "雖多頭趨勢明顯，但 RSI 過熱；建議等待整理或回檔再布局。"
    elif ma20<ma50 and macd_hist<0:
        advice = "趨勢轉空: 建議於反彈至20日均線時分批賣出或觀望。"
    else:
        advice = "訊號混合，建議先觀望，待趨勢明確後再操作。"

    # 完整 commentary_html，研究員觀點不動
    commentary_html = f"""
<h4>研究員觀點</h4>
<p>依據技術指標與模擬結果，{symbol} 在未來 {horizon_key} 期間可能產生較大波動，主要受 μ={mu:.2%}、σ={sigma:.2%} 驅動。</p>
