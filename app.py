...
def simulate_generator(symbol, horizon_key, sims):
    ...
    # 標題及座標軸
    ax.set_title(f"{symbol} 模擬預測 {horizon_key} ({sims} 次模擬)", fontsize=20, fontweight='bold', pad=20)
...
<div style="font-size:1rem; line-height:1.5;">
  ...
  <h4>模擬走勢總結</h4>
  <ul>
    <li>預測股價平均 <strong>{avg_price:.2f} 元</strong>，範圍 {min_price:.2f}–{max_price:.2f} 元，波動度 {vol_pct:.2f}%。</li>
  </ul>
  <h4>指標解讀</h4>
  <ul>
