...
def simulate_generator(symbol, horizon_key, sims):
    ...
    rand = np.random.normal(size=(sims, days))
    increments = (mu - 0.5*sigma**2)*dt + sigma*np.sqrt(dt)*rand
    sims_paths = np.exp(np.cumsum(increments, axis=1)) * current_price
    all_paths = np.hstack([np.full((sims,1), current_price), sims_paths])
    ...
    if ma20>ma50 and macd_hist>0 and rsi14<70:
        advice = "建議逢低分批買入，並於突破高點時加碼；跌破支撐考慮停損。"
    elif ma20>ma50 and rsi14>=70:
        advice = "多頭但 RSI 過熱，建議等待回檔後布局。"
    elif ma20<ma50 and macd_hist<0:
        advice = "逢反彈至20日均線分批賣出或觀望。"
    else:
        advice = "訊號混合，建議觀望至指標更明確。"
...
<div style="font-size:1rem; line-height:1.5;">
  <h4>公司產業與業務</h4>
  <p>屬於 <strong>{industry}</strong>，業務：{summary}</p>
  <h4>模擬走勢總覽</h4>
  <ul>
