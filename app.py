...
def simulate_generator(symbol, horizon_key, sims):
    ...
    ax.plot(
        x, all_paths.mean(axis=0),
        lw=4, color='#dc3545', label='平均路徑'
    )
    ax.set_title(
        f"{symbol} 模擬走勢 {horizon_key} ({sims} 次)",
    ...
    if ma20>ma50 and macd_hist>0 and rsi14<70:
        advice = "建議逢低分批買入，並於突破高點時加碼；跌破下方支撐考慮停損。"
    elif ma20>ma50 and rsi14>=70:
        advice = "多頭趨勢明顯但 RSI 過熱，建議等待回檔再布局。"
    elif ma20<ma50 and macd_hist<0:
        advice = "逢反彈至20日均線分批賣出或觀望，以控風險。"
    else:
        advice = "訊號混合，建議觀望至指標明確後操作。"
...
<div style="font-size:1rem; line-height:1.5;">
  <h4>公司產業與業務</h4>
  <p>屬於 <strong>{industry}</strong> 產業，業務：{summary}</p>
  <h4>模擬結果</h4>
  <ul>
    <li>平均：{avg_price:.2f} 元，低-{min_price:.2f} 元，高-{max_price:.2f} 元，波動 {vol_pct:.2f}%</li>
  </ul>
  <h4>指標解讀</h4>
  <ul>
