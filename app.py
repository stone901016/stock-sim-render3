...
def simulate_generator(symbol, horizon_key, sims):
    ...
    summary_en = info.get('longBusinessSummary', '')[:4000]
    translator = GoogleTranslator(source='auto', target='zh-TW')
    industry = translator.translate(industry_en) if industry_en != 'N/A' else industry_en
    summary = translator.translate(summary_en) if summary_en else '無可用公司簡介'
    ...
    # 3. 模擬最終價 - 向量化一次產生 sims 個 final
    tot_mean = (mu - 0.5 * sigma**2) * days * dt
    tot_sd = sigma * np.sqrt(days * dt)
    finals = current_price * np.exp(
        tot_mean + tot_sd * np.random.normal(size=sims)
    )

    # 4. 小樣本路徑（畫圖用，預設 200 條 or sims 條）
    plot_n = min(sims, 200)
    rand_small = np.random.normal(size=(plot_n, days))
    inc_small = (mu - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * rand_small
    paths_small = current_price * np.exp(np.cumsum(inc_small, axis=1))
    paths_small = np.hstack([np.full((plot_n, 1), current_price), paths_small])
    ...
    macd_hist = (ema12.iloc[-1] - ema26.iloc[-1]) - (
        (ema12 - ema26).ewm(span=9).mean().iloc[-1]
    )
    ...
    ax.plot(x, paths_small.T, lw=0.5, alpha=0.015, color='#007bff')
    ax.plot(x, paths_small.mean(axis=0), lw=4, color='#dc3545', label='平均路徑')
    ax.set_title(f"{symbol} 模擬預測 {horizon_key} ({sims} 次)", fontsize=20, fontweight='bold', pad=20)
    ...
    if ma20 > ma50 and macd_hist > 0 and rsi14 < 70:
        ...
        advice = "雖多頭趨勢明顯，但 RSI 過熱，建議等待回檔後再布局。"
    elif ma20 < ma50 and macd_hist < 0:
        advice = "逢反彈至20日均線時分批賣出或觀望，以控制風險。"
    else:
        advice = "訊號混合，建議觀望至指標更明確後再行操作。"
...
<div style="font-size:1rem; line-height:1.5;">
  ...
  <h4>模擬走勢總覽</h4>
  <ul>
    <li>平均：{avg_price:.2f} 元，範圍 {min_price:.2f}–{max_price:.2f} 元，波動 {vol_pct:.2f}%</li>
  </ul>
  <h4>指標解讀</h4>
  <ul>
    ...
    <li>MACD柱狀={macd_hist:.4f} ⇒ {"多頭動能" if macd_hist>0 else "空頭動能"}</li>
  </ul>
  <h4>建議</h4>
  <ul><li>{advice}</li></ul>
</div>
